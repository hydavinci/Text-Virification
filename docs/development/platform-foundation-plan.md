...
> Historical note: This plan records the foundation work completed before the Monorepo
> migration. Historical file paths, commands, and commit output intentionally retain the
> original `backend/`, `frontend/`, and root `compose.yaml` locations. For current work,
> use `apps/api`, `apps/web`, and `docker compose -f infra/compose.yaml ...`.
...
**Architecture:** During the foundation phase, the existing Flask Demo was preserved under
`translation-pre-checker/` while the new platform was created under `backend/` and
`frontend/`. The approved repository-layout migration later moved those applications to
`apps/api` and `apps/web` and removed the retired Demo. FastAPI handles validation and job
APIs; PostgreSQL is the source of truth for jobs and events; Celery performs background
work; per-job storage owns temporary files; Vue consumes REST and SSE. This plan establishes
domain and adapter interfaces but deliberately does not implement document parsing,
proofreading engines, dictionaries, review decisions, or exports.
...
- During this foundation plan, new code lives outside `translation-pre-checker/` and the
  Demo remains preserved. The approved repository-layout migration removes it later.
...
The task bodies above remain an exact historical implementation record, so their original
paths and command transcripts are intentionally unchanged. Current development uses:

- API source, tests, and Python environment under `apps/api`;
- Web source, tests, and npm workspace under `apps/web`;
- Compose at `infra/compose.yaml`, invoked from the repository root with:
